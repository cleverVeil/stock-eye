...
export async function GET() {
  try {
      ...
      try {
        ...
        if (response.status === 429 || response.status >= 500) {
          ...
          if (retries > maxRetries) {
            lastError = new Error(`API Error: ${response.status} ${response.statusText}. Max retries reached.`);
            break;
          }
          // Exponential backoff
    ...
    if (!response || !response.ok) {
        console.error("Failed to fetch from API after retries:", lastError ? lastError.message : (response ? `${response.status} ${response.statusText}` : "Unknown error"));
        return NextResponse.json({ error: `Failed to fetch API: ${lastError ? lastError.message : (response ? `${response.status} ${response.statusText}` : "Unknown error")}` }, { status: response ? response.status : 500 });
    }
    ...
    try {
        ...
        return NextResponse.json({ error: "Failed to parse JSON response from AI.", rawResponse: jsonText }, { status: 500 });
    }

  } catch (error) {
    console.error("Market Route Error:", error);
    return NextResponse.json({ error: `Failed to fetch market report: ${error.message}` }, { status: 500 });
  }
}
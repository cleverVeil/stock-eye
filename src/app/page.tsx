...
const getInitialData = () => ({
  ...
  marketOutlook: {
    ...
    global: "Fed cut (25 bps) → liquidity positive, but hawkish commentary tempers mood. US-India trade talks rhetoric positive, but 50% tariffs hurting exports. Rupee weak, importers under pressure. (Near all-time low ~88.70/USD)"
  },
  // New section for Global Indices
  globalIndices: [
    { name: "Dow Jones", last: "45,952.24", change: "-301.07", changePercent: "-0.65%" },
    { name: "NASDAQ", last: "17,800.12", change: "+50.45", changePercent: "+0.28%" },
    { name: "FTSE 100", last: "8,300.50", change: "-12.10", changePercent: "-0.15%" },
    { name: "Nikkei 225", last: "39,450.00", change: "+150.20", changePercent: "+0.38%" }
  ],
  ...
  stocksInFocus: {
    positive: ["ITC (Strong outlook)", "L&T (Order book)", "DLF (Pre-sales)", "Ashoka Buildcon (Breakout)"],
...
export default function App() {
    ...
    try {
      ...
      if (candidate && candidate.content?.parts?.[0]?.text) {
        ...
        if (
          ...
          throw new Error("API did not return the required structured data (levels, indices, sectors, or stocks).");
        }
  ...
  const dynamicNiftyChartData = useMemo(() => [
    { time: 'Start', value: niftyLevels.current ? niftyLevels.current - 20 : 25700 },
    { time: 'Mid', value: niftyLevels.current ? niftyLevels.current + 30 : 25750 },
    { time: 'Current', value: niftyLevels.current || 25722 },
  ], [niftyLevels.current]);
  
  const dynamicBankNiftyChartData = useMemo(() => [
    { time: 'Start', value: bankNiftyLevels.current ? bankNiftyLevels.current - 50 : 57700 },
    { time: 'Mid', value: bankNiftyLevels.current ? bankNiftyLevels.current + 80 : 57850 },
  ...
  const sectorOutlook = reportData.sectorOutlook || { positive: [], caution: [] };
  const stocksInFocus = reportData.stocksInFocus || { positive: [], caution: [] };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-4 md:p-8 relative pb-20">
      <header className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Things to Know Today</h1>
      ...
      <main className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* --- Market Outlook --- */}
          <InfoCard title="Market Outlook" icon={<Landmark className="w-6 h-6" />} className="lg:col-span-1">
            <div>
              <h3 className="text-lg font-semibold text-white mb-2 flex items-center"><span className="w-5 h-5 mr-2 rounded-full">🇮🇳</span> India</h3>
              <p><strong className="text-gray-200">Nifty:</strong> {reportData.marketOutlook.india.nifty}</p>
              <p><strong className="text-gray-200">Bank Nifty:</strong> {reportData.marketOutlook.india.bankNifty}</p>
              <p><strong className="text-gray-200">Tone:</strong> {reportData.marketOutlook.india.tone}</p>
            </div>
            <div className="mt-4">
              <h3 className="text-lg font-semibold text-white mb-2 flex items-center"><Globe className="w-5 h-5 mr-2" /> Global</h3>
              <p>{reportData.marketOutlook.global}</p>
            </div>
          </InfoCard>
          
          {/* --- Global Indices (NEW) --- */}
          <InfoCard title="Global Market Snapshot" icon={<Globe className="w-6 h-6" />} className="lg:col-span-1">
            <div className="flow-root">
              <table className="min-w-full divide-y divide-gray-700">
                <thead>
                  <tr>
                    <th scope="col" className="py-2 pr-2 text-left text-sm font-semibold text-white">Name</th>
                    <th scope="col" className="px-2 py-2 text-right text-sm font-semibold text-white">Last</th>
                    <th scope="col" className="px-2 py-2 text-right text-sm font-semibold text-white">Chg. %</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {reportData.globalIndices && reportData.globalIndices.map((index) => {
                    const isNegative = index.change && index.change.startsWith('-');
                    return (
                      <tr key={index.name}>
                        <td className="py-2 pr-2 whitespace-nowrap text-sm font-medium text-gray-200">{index.name}</td>
                        <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-300 text-right">{index.last}</td>
                        <td className={`px-2 py-2 whitespace-nowrap text-sm font-medium text-right ${isNegative ? 'text-red-400' : 'text-green-400'}`}>
                          {index.changePercent}
                        </td>
                      </tr>
                    );
          ...
          <InfoCard title="Trading View" icon={<Target className="w-6 h-6" />}>
            ...
            <h3 className="text-lg font-semibold text-white mt-3">Bank Nifty</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* --- Charts --- */}
          <InfoCard title="Index Charts" icon={<TrendingUp className="w-6 h-6" />} className="lg:col-span-2">
            <p className="text-sm text-gray-400 mb-4">Illustrative charts showing current value against key API-provided levels.</p>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              
              {/* Nifty 50 Chart */}
              <div className="w-full h-64 md:h-80">
                <h3 className="text-lg font-semibold text-white mb-2 text-center">Nifty 50</h3>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dynamicNiftyChartData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="value" name="Nifty 50" stroke="#818CF8" strokeWidth={2} dot={true} />
                    
                    {/* Support and Resistance Lines */}
                    {niftyLevels.support1 && <ReferenceLine y={niftyLevels.support1} label={{ value: `S1 (${niftyLevels.support1})`, position: 'insideBottomLeft', fill: '#CA8A04' }} stroke="#CA8A04" strokeDasharray="3 3" />}
                    {niftyLevels.support2 && <ReferenceLine y={niftyLevels.support2} label={{ value: `S2 (${niftyLevels.support2})`, position: 'insideBottomLeft', fill: '#CA8A04' }} stroke="#CA8A04" strokeDasharray="5 5" />}
                    {niftyLevels.resistance1 && <ReferenceLine y={niftyLevels.resistance1} label={{ value: `R1 (${niftyLevels.resistance1})`, position: 'insideTopLeft', fill: '#16A34A' }} stroke="#16A34A" strokeDasharray="3 3" />}
                    {niftyLevels.resistance2 && <ReferenceLine y={niftyLevels.resistance2} label={{ value: `R2 (${niftyLevels.resistance2})`, position: 'insideTopLeft', fill: '#16A34A' }} stroke="#16A34A" strokeDasharray="5 5" />}
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              {/* Bank Nifty Chart */}
              <div className="w-full h-64 md:h-80">
                <h3 className="text-lg font-semibold text-white mb-2 text-center">Bank Nifty</h3>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dynamicBankNiftyChartData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#4B5563" />
                    <XAxis dataKey="time" stroke="#9CA3AF" />
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: '#F9FAFB' }}
                    />
                    <Line type="monotone" dataKey="value" name="Bank Nifty" stroke="#F472B6" strokeWidth={2} dot={true} />
                    
                    {/* Support and Resistance Lines */}
                    {bankNiftyLevels.support1 && <ReferenceLine y={bankNiftyLevels.support1} label={{ value: `S1 (${bankNiftyLevels.support1})`, position: 'insideBottomLeft', fill: '#CA8A04' }} stroke="#CA8A04" strokeDasharray="3 3" />}
                    {bankNiftyLevels.support2 && <ReferenceLine y={bankNiftyLevels.support2} label={{ value: `S2 (${bankNiftyLevels.support2})`, position: 'insideBottomLeft', fill: '#CA8A04' }} stroke="#CA8A04" strokeDasharray="5 5" />}
                    {bankNiftyLevels.resistance1 && <ReferenceLine y={bankNiftyLevels.resistance1} label={{ value: `R1 (${bankNiftyLevels.resistance1})`, position: 'insideTopLeft', fill: '#16A34A' }} stroke="#16A34A" strokeDasharray="3 3" />}
                    {bankNiftyLevels.resistance2 && <ReferenceLine y={bankNiftyLevels.resistance2} label={{ value: `R2 (${bankNiftyLevels.resistance2})`, position: 'insideTopLeft', fill: '#16A34A' }} stroke="#16A34A" strokeDasharray="5 5" />}
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {/* --- Sector Outlook (NEW) --- */}
          <InfoCard title="Sector Outlook" icon={<TrendingUp className="w-6 h-6" />} className="lg:col-span-1">
            <h3 className="text-lg font-semibold text-green-400 mb-2">Positive</h3>
            <StockList items={sectorOutlook.positive} type="positive" />
            <h3 className="text-lg font-semibold text-yellow-400 mt-4 mb-2">Caution</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <InfoCard title="Stocks in Focus" icon={<Target className="w-6 h-6" />} className="lg:col-span-1">
            <h3 className="text-lg font-semibold text-green-400 mb-2">Positive</h3>
          ...
          {/* --- In-Depth Analysis --- */}
          <InfoCard title="In-Depth Market Analysis" icon={<Newspaper className="w-6 h-6" />} className="lg:col-span-2">
        ...
        {/* --- Sources --- */}
        {sources.length > 0 && (
          <InfoCard title="Research Sources" icon={<ExternalLink className="w-6 h-6" />}>
